...
// -> If api call is failed due to some reason and When 'catch(cb)' function is executed in call stack, its only work is to move its argument which is a callback function into the onRejection Array.

// -> Now when fetch() fn has done its api call and got back the response from url endpoint, it sets the response to the value property in the Heap memory's Promise Object, if api call is failed it populates value property with error details.
...
// -> ThereAfter, Event loop continues its usual work of pushing these callback fns into the call stack whenever call stack is empty.
